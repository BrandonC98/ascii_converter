...
mod tests {
    ...
    mod string_to_hexadecimal_tests {
        ...
        fn string_to_hexadecimal_test_unhappy_path() {
            ...
            assert_eq!(
                string_to_hexadecimal(input),
                Err("A character in the string isn't apart of the ascii table".to_string())
            );
        }
    }
    ...
    mod decimal_to_hexadecimal_tests {
        ...
        fn decimal_to_hexadecimal_test_unhappy_path() {
            ...
            assert_eq!(
                ...
                Err("this function doesn't support values over 126".to_string())
            );
        }
    ...
    mod string_to_decimal_tests {
        ...
        fn string_to_decimals_test_happy_none_alphabetical() {
            let expected = vec![
                49, 50, 51, 52, 53, 54, 55, 56, 57, 48, 32, 33, 36, 37, 94, 38, 42, 40, 41, 45, 95,
                61, 43, 123, 125, 91, 93, 59, 58, 64, 39, 126, 35, 60, 44, 46, 62, 47, 63,
            ];

            assert_eq!(
                string_to_decimals("1234567890 !$%^&*()-_=+{}[];:@'~#<,.>/?"),
                Ok(expected)
            );
        }
        ...
        fn string_to_decimals_test_happy_alphabetical() {
            let expected = vec![
                113, 119, 101, 114, 116, 121, 117, 105, 111, 112, 97, 115, 100, 102, 103, 104, 106,
                107, 108, 122, 120, 99, 118, 98, 110, 109, 81, 87, 69, 82, 84, 89, 85, 73, 79, 80,
                65, 83, 68, 70, 71, 72, 74, 75, 76, 90, 88, 67, 86, 66, 78, 77,
            ];

            assert_eq!(
                string_to_decimals("qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM"),
                Ok(expected)
            );
        }
    ...
    mod binary_to_decimal_tests {
        ...
        fn binary_to_decimal_test_unhappy_path() {
            ...
            assert_eq!(
                ...
                Err("number passed in isn't binary".to_string())
            );
        }
    }
    ...
    mod decimals_to_string_tests {
        ...
        fn decimals_to_string_test_unhappy_path() {
            ...
            assert_eq!(
                decimals_to_string(&input),
                Err("the number is outside the ascii range".to_string())
            );
        }
    }
